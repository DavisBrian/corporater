...
You may have a more complicated look-up table which has multiple columns of information. Suppose we have a vector of integer grades, and a table that describes their properties: 
...
1. `which()` allows you to convert a boolean representation to an integer representation. There's no reverse operation in base R.  Create an `unwhich` function. `unwhich(which(x), length(x))` should return your original vector. 
...
2. Install the tidyverse package, if you already have it installed upgrade to the latest version.  This can be done by either typing `install.packages("tidyverse")` in the console or by using the "Packages"" tab inside RStudio.
...
3. Read and do the exercises in Chapters 1-3 of[R for Data Science](http://r4ds.had.co.nz/).  I highly recommend reading [A Layered Grammar of Graphics](http://vita.had.co.nz/papers/layered-grammar.pdf).