...
- `??topic` searches the documention for _topic_.
...
__Messy code hides bugs__

```{r, echo = FALSE, out.width = NULL}
knitr::include_graphics("images/funs.png", dpi = 300)
```
...
From the [R Inferno](http://www.burns-stat.com/pages/Tutor/R_inferno.pdf):
...
- the environment this function lives in is the global environement. (We'll discuss environments more in the next section.)
...
By defualt the last line of the function is returned.  Thus, there is no reason to explicitly call `return`, unless you are returning from the function early.  Inside functions use `stop` to return error messages, `warning` to return warning messages, and `message` to print a message to the console.
...
2. Vectorization / function practice.
...
$ \sum_\limits_{n=0}^{\infty} \frac{(-1)^n}{2*n+1} = \frac{\pi}{4}$
...
Your take is to vectorize this function.  Do not use any looping or apply functions.  This one is a bit tricky.  Hint: It may be easier to think about it in terms of the series notation and not the summation notation.
