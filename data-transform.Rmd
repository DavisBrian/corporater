...
You can convert a data frame to a tibble with `as_tibble()`, while you con coerce a tibble to a data frame with `as.data.fame()`.
...
-    everything(): all varaibles or everything else not already selected / deselected.
...
Typically it takes a series of operations to go from raw data to meaningful anlaysis. There are (at least) four ways to to do this:
...
Simplest approach is to create a new object at each step.  This method tends to clutter the code with unimportant, and uninformative names.  Also, each object eats up memory, whcih becomes an issue as data set sizes grow large.

Rule of thumb:  If there is a natrual new name it may be an appropriate method.  If the natrual name is to increment a counter on the end of the original varaible a different method is generally preferable.
...
```{block type='rmdnote'}
A common method is to combine the abve methods and use an temporary varaible name for the intermediate steps such a "tmp"" or ".".
```
...
```{block type='rmdnote'}
This method is generally not appropriate if you need to next more than two functions together.  However, it is appropriate in many cases. (e.g. sum(is.na(x)) to find the number of missing values in x)
```

### Uwe the pipe operator `%>%`

Finally, we can use the pipe.  This is my favourite form, because it focusses on verbs (what we are doing), not nouns (what we are doing it on). You can read this series of function compositions like it’s a set of imperative actions.
...
2.    Functions that use lazy evaluation. In R, function arguments are only computed when the function uses them, not prior to calling the function. The pipe computes each element in turn, so you can’t rely on this behaviour. (e.g. `tryCatch`, `try`, `suppressMessages`)
